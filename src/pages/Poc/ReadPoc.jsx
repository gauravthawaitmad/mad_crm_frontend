...
export default function ReadPoc() {

  const translate = useLanguage();
  const { result: currentItem } = useSelector(selectCurrentItem);

  console.log('current item in read poc box :', currentItem);
    ...
    <Descriptions bordered column={1}>
      ...
      {currentItem.partner_name && (
        <Descriptions.Item label={translate('partner_name')}>
          {translate(currentItem?.partner_name)}
        </Descriptions.Item>
      )}

      {currentItem.poc_name && (
        <Descriptions.Item label={translate('poc_name')}>{currentItem?.poc_name}</Descriptions.Item>
      )}
      ...
      <Descriptions.Item label={translate('partner_affiliation_type')}>
        {currentItem?.partner_affiliation_type}
      </Descriptions.Item>

      <Descriptions.Item label={translate('co_name')}>{currentItem?.co_name}</Descriptions.Item>
